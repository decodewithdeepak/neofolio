...
const Project = memo(({ project, index }) => {
  // Memoize project content to prevent recalculations during re-renders
  ...
  }, [project, index]); // Dependencies for useMemo
  ...
  return (
    <motion.div
      ...
      className={`transform-gpu will-change-transform contain-content flex flex-col md:flex-row ${index % 2 === 1 ? "md:flex-row-reverse" : ""}
...
const ProjectCard = memo(({ projects = [] }) => {
  // Wrap all projects in useMemo to prevent recreating the array on re-renders
